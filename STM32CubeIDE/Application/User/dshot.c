...
/* Variables */
static uint16_t motor1_dmabuffer[DSHOT_DMA_BUFFER_SIZE];//  	jimmy edit 25/05/20
...
static void dshot_prepare_dmabuffer(uint16_t* motor_dmabuffer, uint16_t value);//  	jimmy edit 25/05/20
...
static void dshot_set_timer(dshot_type_e dshot_type)
{
	...
	dshot_prescaler = lrintf((float) timer_clock / dshot_choose_type(dshot_type) + 0.01f) - 1;//dshot600: 72mhz/12mhz +0.01 -1 =5
...
static void dshot_dma_tc_callback(DMA_HandleTypeDef *hdma)
{
	...
	else if(hdma == htim->hdma[TIM_DMA_ID_UPDATE])
		{
...
//	MOTOR_1_TIM->hdma[TIM_DMA_ID_UPDATE]->XferCpltCallback = dshot_dma_tc_callback;

	MOTOR_1_TIM->hdma[TIM_DMA_ID_CC3]->XferCpltCallback = dshot_dma_tc_callback;
...
//	HAL_TIM_PWM_Start(MOTOR_4_TIM, MOTOR_4_TIM_CHANNEL);
  	...
  	//  	jimmy edit 25/05/20
    // 啟用 CH3 preload
    __HAL_TIM_ENABLE_OCxPRELOAD(MOTOR_1_TIM, MOTOR_1_TIM_CHANNEL);
    // 立即產生一次 Update，把 preload 的初值載入 CCR3
//    TIM_EVENTSOURCE_CC3
    HAL_TIM_GenerateEvent(MOTOR_1_TIM, TIM_EVENTSOURCE_CC3);

//    HAL_TIM_GenerateEvent(MOTOR_1_TIM, TIM_EVENTSOURCE_UPDATE);
    //  	jimmy edit 25/05/20
}
...
static void dshot_dma_start()
{
//	HAL_DMA_Start_IT(MOTOR_1_TIM->hdma[TIM_DMA_ID_UPDATE], (uint32_t)motor1_dmabuffer, (uint32_t)&MOTOR_1_TIM->Instance->CCR3, DSHOT_DMA_BUFFER_SIZE);//jimmy edit

	HAL_DMA_Start_IT(MOTOR_1_TIM->hdma[TIM_DMA_ID_CC3], (uint32_t)motor1_dmabuffer, (uint32_t)&MOTOR_1_TIM->Instance->CCR3, DSHOT_DMA_BUFFER_SIZE);//jimmy edit
...
static void dshot_enable_dma_request()
{
//	__HAL_TIM_ENABLE_DMA(MOTOR_1_TIM, TIM_DMA_UPDATE);

	__HAL_TIM_ENABLE_DMA(MOTOR_1_TIM, TIM_DMA_CC3);
